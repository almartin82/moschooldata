# ==============================================================================
# LIVE Pipeline Tests for moschooldata
# ==============================================================================
#
...
test_that("Missouri DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Missouri DOE URL and verify HTTP 200")
})
...
test_that("Can download Missouri enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Missouri enrollment file", {
  skip_if_offline()
...
test_that("moschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- moschooldata::get_available_years()
  ...
  tryCatch({
    raw <- moschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- moschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- moschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- moschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- moschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- moschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- moschooldata:::get_cache_path(2024, "enrollment")
