...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = mo.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pymoschooldata as mo
        assert hasattr(mo, 'fetch_enr')
        assert hasattr(mo, 'fetch_enr_multi')
        assert hasattr(mo, 'tidy_enr')
        assert hasattr(mo, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pymoschooldata as mo
        assert hasattr(mo, '__version__')
        assert isinstance(mo.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = mo.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = mo.fetch_enr(years['max_year'])
        # Total enrollment in Missouri varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Missouri should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = mo.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = mo.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pymoschooldata as mo
        years = get_test_years()
        df_single = mo.fetch_enr(years['max_year'])
        df_multi = mo.fetch_enr_multi([years['max_year']])
