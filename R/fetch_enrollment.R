...
#' Fetch Missouri enrollment data
#'
#' Downloads and processes enrollment data from the Missouri Department of
#' Elementary and Secondary Education (DESE) via the Missouri Comprehensive
#' Data System (MCDS).
#'
...
fetch_enr_multi <- function(end_years, tidy = TRUE, use_cache = TRUE) {
  ...
  # Validate years
  avail <- get_available_years()
  invalid_years <- end_years[end_years < avail$min_year | end_years > avail$max_year]
